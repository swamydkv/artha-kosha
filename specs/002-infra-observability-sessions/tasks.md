...
- [ ] T014 Implement structured logging middleware with request/correlation ID generation in `apps/finance-api/internal/http/middleware/logging.go`
- [ ] T015 Implement recovery middleware for panic handling in `apps/finance-api/internal/http/middleware/recovery.go`
- [ ] T016 Implement timeout middleware for request timeout enforcement in `apps/finance-api/internal/http/middleware/timeout.go`
- [ ] T017 Implement CORS middleware with go-chi/cors in `apps/finance-api/internal/http/middleware/cors.go`
- [ ] T018 Implement password hashing utilities with alexedwards/argon2id in `apps/finance-api/internal/auth/security.go` (replace existing with Argon2id)
- [ ] T019 Implement authentication abstraction layer for future OIDC migration in `apps/finance-api/internal/auth/provider.go`
- [ ] T020 Update middleware chain order in `apps/finance-api/internal/http/router.go` (Request ID → Logging → Recovery → CORS → Timeout → Auth → Router)
- [ ] T021 Create transaction wrapper for ACID compliance in `apps/finance-api/internal/database/transaction.go`
- [ ] T022 Create audit service interface and basic implementation in `apps/finance-api/internal/audit/audit_service.go`
- [ ] T023 Create domain event service interface and basic implementation in `apps/finance-api/internal/events/domain_event_service.go`
- [ ] T024 Create outbox service interface and basic implementation in `apps/finance-api/internal/events/outbox_service.go`
