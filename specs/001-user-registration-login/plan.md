...
The authentication provider will be abstracted behind a dedicated authentication module so that the MVP can use a local username/password provider while remaining extensible to OpenID Connect and Keycloak in production without changing business logic.
...
**Performance Goals**: Registration must complete within 2 seconds and login within 1 second in the local benchmark environment; benchmark results must be captured in the validation stage.

**Constraints**: Authentication must stay session-based for the MVP, passwords must never be stored in plaintext, the authentication provider must remain behind a dedicated abstraction layer, and no financial features may be introduced in this slice
...
**Structure Decision**: The repository will use the constitution-defined layout with a Go modular backend under `apps/finance-api`, a Next.js TypeScript client under `apps/web`, API contracts under `api/`, and local infrastructure under `infra/`. The backend will separate domain modules from shared infrastructure and will store migrations in a dedicated `migrations` directory plus `sqlc` query definitions in a dedicated `sql` directory. All API contracts for this repository must live directly under `api/` and not in feature-specific folders within `specs/`.
