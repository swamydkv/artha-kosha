...
func (r *SQLRepository) CreateTransaction(ctx context.Context, req CreateTransactionRequest) (*Transaction, error) {
	...
	if err := q.InsertTransaction(ctx, sqlc.InsertTransactionParams{ID: id, AccountID: uuid.MustParse(req.AccountID), Amount: req.Amount, Memo: memo}); err != nil {
		return nil, err
	}
	return &Transaction{TransactionID: id.String(), AccountID: req.AccountID, Amount: req.Amount, Memo: req.Memo}, nil
}
