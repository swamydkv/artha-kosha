...
func (s *Service) EmitTx(ctx context.Context, tx *sql.Tx, e DomainEvent) error {
	...
	if inserter, ok := interface{}(s.repo).(interface{ InsertTx(context.Context, *sql.Tx, DomainEvent) error }); ok {
	...
	// attempt to use outbox repo InsertTx via the outbox service's repo if exposed
	if rw, ok := interface{}(s.outbox).(interface{ repo() interface{} }); ok {
		// not relying on this helper; try to use common pattern via type assertion on outbox.Service's underlying repo
	}

	// best-effort: if outbox service's repository supports InsertTx, use that via an exported method on outbox package
	if inserter, ok := interface{}(s.outbox).(interface{ EnqueueTx(context.Context, *sql.Tx, outbox.OutboxEntry) error }); ok {
