...
// InsertTx inserts a domain event using the provided transaction.
func (r *SQLRepository) InsertTx(ctx context.Context, tx *sql.Tx, e DomainEvent) error {
	q := `INSERT INTO domain_events (id, event_type, aggregate_id, aggregate_type, event_data, timestamp, processing_status, retry_count) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`
	_, err := tx.ExecContext(ctx, q, e.ID, e.EventType, e.AggregateID, e.AggregateType, e.EventData, e.Timestamp, e.ProcessingStatus, e.RetryCount)
	return err
}
