...
1. **Given** a user has not created an account, **When** they submit a valid registration form with unique username, email, and mobile number, **Then** the system creates the account, establishes the authenticated session, and presents the logged-in home state.
...
- A successful registration will automatically establish the authenticated session and present the logged-in home state, matching the clarified post-registration behavior for this MVP.
