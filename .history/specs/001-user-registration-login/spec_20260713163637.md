...
- **Primary components**: Landing page, registration screen, login screen, authenticated home screen, logout action, and an authentication service layer that validates credentials and maintains session state.
- **Core responsibilities**: The front-end handles user interaction and screen transitions. The back-end handles account creation, credential validation, session establishment, and logout. The database persists the user record atomically and stores password hashes only.
- **Interaction boundaries**: UI actions trigger authentication API calls. The backend performs validation, records the account, and returns either a success result or a generic authentication error.

### User Flow

1. **Given** a user is on the landing page, **When** they choose Create Account, **Then** they are taken to the registration screen.
2. **Given** a user completes the registration form with valid values, **When** the form is submitted, **Then** the system validates the fields and creates the account record within a single transaction.
3. **Given** registration succeeds, **When** the operation completes, **Then** the user is redirected to the login page.
4. **Given** a registered user enters a valid username and password, **When** they submit the login form, **Then** the system validates the credentials and establishes an authenticated session.
5. **Given** the user is authenticated, **When** they reach the home view, **Then** the screen shows a welcome message using the user’s first name and a logout action.
6. **Given** the user chooses logout, **When** the session is terminated, **Then** the system returns the user to a signed-out session-ended state.

### Call Flow

1. **Client** → **Auth UI**: User clicks Create Account or Login.
2. **Auth UI** → **Auth API**: Submit registration or login request with entered form values.
3. **Auth API** → **User Service / Repository**: Validate input, enforce uniqueness rules, hash the password, and persist or retrieve the user record.
4. **User Service / Repository** → **PostgreSQL**: Execute the registration or credential lookup within an ACID transaction.
5. **Auth API** → **Client**: Return success, redirect, or generic authentication error.
6. **Client** → **Auth UI**: Present the next screen state: login page, authenticated home, or signed-out session-ended state.
...
- **FR-017**: The system MUST maintain a clear onboarding and sign-in experience that supports responsive single-column entry and inline validation feedback.
...
- **SC-002**: At least 99% of successful sign-ins reach the logged-in home state without a manual retry.
